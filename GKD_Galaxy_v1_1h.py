...
class GKD_Galaxy_v1_1h(IStrategy):
    ...
    def _get_initial_stake(self, trade: Trade) -> float:
        ...
        Prioritizes stored value in custom_data if available.
        """
        # 1. Prioritize the stored value to avoid compounding/recalculation issues
        stored_stake = trade.get_custom_data('initial_base_stake')
        if stored_stake is not None:
            return float(stored_stake)

        # 2. Use Config Stake Amount if fixed (User Request: Stop compounding)
        # This acts as a reliable fallback for old trades where custom_data is missing
        if self.config.get('stake_amount') != 'unlimited':
            try:
                fixed_stake = float(self.config['stake_amount'])
                return fixed_stake
            except Exception:
                pass

        try:
            # 3. Fallback: Get first buy order (or sell for short)

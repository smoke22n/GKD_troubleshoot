...
class GKD_Galaxy_v1_1h(IStrategy):
    ...
    def order_filled(self, pair: str, trade: Trade, order: 'Order', current_time: datetime, **kwargs) -> None:
        ...
        """
        # 🚀 STORE INITIAL STAKE ON FIRST ENTRY (Non-Compounding Base)
        # Note: custom_stake_amount cannot access trade object for new trades, so we store it here.
        if order.ft_order_side == 'buy' and not trade.is_short: # Long Entry
             if 'initial_base_stake' not in trade.get_custom_data():
                 # For the first order, trade.stake_amount is the initial stake
                 trade.set_custom_data('initial_base_stake', trade.stake_amount)
                 logger.info(f"💾 {pair}: Saved initial_base_stake={trade.stake_amount:.2f} USD")
        elif order.ft_order_side == 'sell' and trade.is_short: # Short Entry
             if 'initial_base_stake' not in trade.get_custom_data():
                 trade.set_custom_data('initial_base_stake', trade.stake_amount)
                 logger.info(f"💾 {pair}: Saved initial_base_stake={trade.stake_amount:.2f} USD")
